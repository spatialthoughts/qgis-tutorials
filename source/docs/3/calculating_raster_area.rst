...
29. You can also change the order of fields in the table using the :guilabel:`Move Selected Fields` button. Once you are done with the edits, click on the ``...`` button next to :guilabel:`Refactored`, and choose :guilabel:`Save To File...`. Enter the name as ``park_area_by_landcover.xlxs``. Click :guilabel:`Run`. 
